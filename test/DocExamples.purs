...
main = void $ launchAff $ do
  result <- ADN.request (AX.defaultRequest { url = "/api", method = Left GET, responseFormat = ResponseFormat.json })
  case result of
...
main' :: Effect Unit
main' = void $ launchAff $ do
  result1 <- ADN.get ResponseFormat.json "/api"
  case result1 of
    Left err -> log $ "GET /api response failed to decode: " <> AX.printError err
    Right response -> log $ "GET /api response: " <> J.stringify response.body

  result2 <- ADN.post ResponseFormat.json "/api" (Just (RequestBody.json (J.fromString "test")))
  case result2 of
